...
sketchy::load_packages(packages = c("knitr", "formatR", "rprojroot", "sp", "rgdal", "rgeos", "dplyr", "raster", "dplyr", "spData", "vegan", "ggplot2", "pbapply", "viridis", "caret", github = "PhenotypeSpace", "brms", github = "brmsish", "ecodist", "pbapply"))
...
```{r, fig.cap="Types of vocal geographic variation that were simulated", out.height="100%"}
# seed to make it reproducible
set.seed(123)
...
lng_data <- stack(data[, c("clinal", "dialect", "dialect_clinal", "random")])
...
ggplot(lng_data, aes(x = lon_seq, y = values)) +
    geom_line(color = "#3E4A89FF", size = 1.6) + 
...
# regression models
# set data format
rect_var <- cbind(clinal_dist = as.dist(scale(clinal_dist)), dialect_dist = as.dist(scale(dialect_dist)), dialect_clinal_dist = as.dist(scale(dialect_clinal_dist)), random_dist = as.dist(scale(random_dist)), geo_dist = as.dist(scale(geo_dist)), call_member_binary = call_member_binary)
...
(mod_dialect <- MRM2(formula = dialect_dist ~ geo_dist + call_member_binary, nperm = nperm, mat = rect_var[, c("dialect_dist", "geo_dist", "call_member_binary")]))
...
(mod_clinal <- MRM2(formula = clinal_dist ~ geo_dist + call_member_binary, nperm = nperm, mat = rect_var[, c("clinal_dist", "geo_dist", "call_member_binary")]))
...
# model predicting clinal variation
(mod_dialect_clinal <- MRM2(formula = dialect_clinal_dist ~ geo_dist + call_member_binary, nperm = nperm, mat = rect_var[, c("dialect_clinal_dist", "geo_dist", "call_member_binary")]))
...
(mod_random <- MRM2(formula = random_dist ~ geo_dist + call_member_binary, nperm = nperm, mat = rect_var[, c("random_dist", "geo_dist", "call_member_binary")]))
...
rep_models <- pbreplicate(reps, cl = 20, expr = {
    ...
    names(mods) <-
        c("Clinal", "Dialect", "Dialect * clinal", "Random")
...
rect_var <- cbind(acous_sim = as.dist(scale(1- xc)), geo_dists = as.dist(scale(geo_dists)), call_member_binary = call_member_binary)
...
#  
# saveRDS(mod, "./data/processed/matrix_correlation_distance_vs_acoustic_similarity.RDS")
...
mod <- readRDS("./data/processed/matrix_correlation_distance_vs_acoustic_similarity.RDS")
